"use client"
import { DialogDescription, DialogContent, DialogHeader, DialogTitle, DialogClose } from "@/components/ui/dialog";
    ...
    return (
        <DialogContent showCloseButton={false} className="shadow rounded-xl w-full border-none p-6">
            <DialogHeader className="flex flex-col gap-9">
                <DialogTitle className="text-wrap w-full ">үйлчилгээ авах ажилтанг өөрчлөх гэж байна.</DialogTitle>
                <DialogDescription className="flex w-fit gap-10 justify-between ">{
                    company?.employees.map((item, index) => (
                        <span key={index} className={item.employeeName == isSelectEmployee ? ` relative flex-1  justify-center w-full p-5 rounded-xl bg-clip-border md:bg-clip-padding border  border-sky-600` : " relative flex-1 justify-center w-full p-5 rounded-xl bg-clip-border md:bg-clip-padding border border-gray-400"}
                            onClick={() => { if (isSelectEmployee === "" || isSelectEmployee !== item.employeeName) setIsSelectEmployee(item.employeeName) }}>
                            {item.availability == true && <DialogClose>
